...
class ExportStratController {
	...
	def export() {
		doOutside {
			try {
				// get the ModelContainer for the selected strat column section
				def container = app.controllers['exportStratSections'].getSelectedContainer()
				def containerName = app.controllers['exportStratSections'].getSelection()

				view.exportBtn.enabled = false // disable to prevent starting a parallel Export
				view.progressMessage.text = "Exporting ${containerName}..."
				...
				Dialogs.showMessageDialog("Error", "Exception: ${e.message}", view.root)
				view.progressMessage.text = "Export failed."
			}
